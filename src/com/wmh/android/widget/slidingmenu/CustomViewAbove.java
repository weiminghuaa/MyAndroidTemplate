package com.wmh.android.widget.slidingmenu;
...
import com.wmh.android.widget.slidingmenu.SlidingMenu.OnClosedListener;
import com.wmh.android.widget.slidingmenu.SlidingMenu.OnOpenedListener;

//import com.wmh.android.slidingmenu.SlidingMenu.OnCloseListener;
//import com.wmh.android.slidingmenu.SlidingMenu.OnOpenListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	private boolean mIsActionInViewPager = false;
	...
	protected Rect mViewPagerRect = null;

	public void setViewPagerRectAbove(Rect rect) {
		mViewPagerRect = rect;
	}
	...
	private boolean thisTouchAllowed(MotionEvent ev) {
		...
		if (isMenuOpen()) {
			...
			case SlidingMenu.TOUCHMODE_ENABLE_LEFT_SLIDING_IN_VIEWPAGER:
				return true;
			case SlidingMenu.TOUCHMODE_ENABLE_RIGHT_SLIDING_IN_VIEWPAGER:
				return true;
			case SlidingMenu.TOUCHMODE_NONE_SLIDING_IN_VIEWPAGER:
				return true;
			}
		}
		return false;
	...
	private boolean thisSlideAllowed(float dx) {
		...
		if (isMenuOpen()) {
			...
			if (!mIsActionInViewPager) {
				allowed = mCustomViewBehind.menuClosedSlideAllowed(dx);
			} else {
				if (mCustomViewBehind.getMode() == SlidingMenu.LEFT_RIGHT) {
					if (mTouchMode == SlidingMenu.TOUCHMODE_ENABLE_LEFT_SLIDING_IN_VIEWPAGER) {
						allowed = dx > 0;
					} else if (mTouchMode == SlidingMenu.TOUCHMODE_ENABLE_RIGHT_SLIDING_IN_VIEWPAGER) {
						allowed = dx < 0;
					} else if (mTouchMode == SlidingMenu.TOUCHMODE_NONE_SLIDING_IN_VIEWPAGER) {
						allowed = false;
					}
				} else {
					allowed = mCustomViewBehind.menuClosedSlideAllowed(dx);
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (thisTouchAllowed(ev)) {
				...
				if (isMenuOpen())
					return true;
				if (mViewPagerRect != null) {
					float eventX = ev.getX();
					float eventY = ev.getY();
//					Log
//							.e("aa", "view pager rect is not null. eventX:" + eventX + "eventY:" + eventY
//									+ "mViewPagerRect.left" + mViewPagerRect.left + "mViewPagerRect.right"
//									+ mViewPagerRect.right + "mViewPagerRect.top" + mViewPagerRect.top
//									+ "mViewPagerRect.bottom" + mViewPagerRect.bottom);
					if ((eventX >= mViewPagerRect.left) && (eventX <= mViewPagerRect.right)
							&& (eventY > mViewPagerRect.top) && (eventY < mViewPagerRect.bottom)) {
//						Log.e("aa", "touch action down in view pager rect");
						mIsActionInViewPager = true;
					} else {
						mIsActionInViewPager = false;
					}
				} else {
					mIsActionInViewPager = false;
				}
